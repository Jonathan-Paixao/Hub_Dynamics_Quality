...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function removeLegacyAutomationModals() {
        const legacyModalIds = ['login-modal-overlay', 'confirm-modal-overlay'];
        ...
        const orphanLegacyModal = document.querySelector(
            '.modal-content:has(#modal-user):has(#modal-pass):has(#modal-execute-btn)'
        );
        if (orphanLegacyModal) {
            const parentOverlay = orphanLegacyModal.closest('.modal-overlay');
            if (parentOverlay) {
                parentOverlay.remove();
            } else {
                orphanLegacyModal.remove();
            }
        }
    }

    removeLegacyAutomationModals();
