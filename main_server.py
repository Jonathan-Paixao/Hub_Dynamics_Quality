...
try:
    ...
    Llama = None
    print('AVISO: llama-cpp-python não instalado. O assistente estará indisponível.')
import glob
...
def filter_dashboards_by_area(dashboards_data, user_area, is_guest=False):
    """Retorna uma cópia dos dashboards com apenas os itens do hub_area do usuário.
    Se is_guest=True, retorna apenas dashboards marcados como 'general'.
    Remove áreas que ficam vazias após a filtragem."""
    import copy
    filtered = copy.deepcopy(dashboards_data)
    for system_key, system_data in filtered.items():
...
def dashboards():
    ...
    dashboards_data = filter_dashboards_by_area(all_dashboards_data, user_area, is_guest=is_guest)
    # --- FIM DA FILTRAGEM ---
...
def chatbot_query():
    ...
    # Monta o system prompt a partir do prompt.json
    try:
